...
func (c *Client) Liveness(reqCtx clients.ApiRequestContext) error {
	const semLogContext = "cms-api-client::liveness"

	const ApiUrl = "/health/liveness"

	ep := c.livenessUrl(ApiUrl, reqCtx.Dom, reqCtx.Ns, reqCtx.Lang, nil)
	...
	harEntry, err := c.client.Execute(req,
		restclient.ExecutionWithOpName("api-cms-liveness"),
...
func (c *Client) livenessUrl(apiPath string, domain, site, lang string, qParams []har.NameValuePair) string {
